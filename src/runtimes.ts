...
const BinaryFileExtensions: Set<RuntimeFileExtension> =
  new Set<RuntimeFileExtension>(['zip', 'jar']);

// Send HTTP request to platform endpoint for runtimes configuration
export async function fromPlatform(
  ...
  try {
    ...
    if (err.response) {
      throw new Error(
        `http request failed (${url}) with status (${err.response.status}): ${err.response.statusText} `
      );
...
export function runtimeForFileExtension(
  fileExtension: RuntimeFileExtension
): RuntimeKind | undefined {
  const runtime = Object.entries(FileExtensionRuntimes).find((item) =>
    item[1].includes(fileExtension)
  );
  return runtime && runtime.length > 0 ? `${runtime[0]}:default` : undefined;
}

// Does file extension imply binary data?
export function isBinaryFileExtension(
  fileExtension: RuntimeFileExtension
): boolean {
...
export function fileExtensionForRuntime(
  runtime: RuntimeKind,
  isBinaryExtension: boolean
): RuntimeFileExtension {
  debug_log(
    `fileExtensionForRuntime: runtime (${runtime}) binary (${isBinaryExtension})`
  );
  ...
  return isBinaryExtension
    ? FileExtensionRuntimes[runtime].filter((item) =>
        BinaryFileExtensions.has(item)
      )[0]
    : FileExtensionRuntimes[runtime][0];
}
...
export async function canonicalRuntime(
  runtime: RuntimeKind
): Promise<RuntimeKind> {
...
function isValidRuntimesJson(
  src: Record<string, unknown>
): src is RuntimesConfig {
  return Object.values(src).every(
    (v) => Array.isArray(v) && v.every((k) => k.hasOwnProperty('kind')) // eslint-disable-line no-prototype-builtins
  );
}
...
export async function load(apihost: string): Promise<RuntimesConfig> {
  ...
  if (!isValidRuntimesJson(runtimes)) {
    throw new Error(
      `Invalid runtime JSON received from platform API: ${apihost}`
    );
  }
  return runtimes;
}
