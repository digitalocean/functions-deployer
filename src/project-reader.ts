...
export async function readTopLevel(
  ...
  return reader.readdir('').then((items) => {
    ...
    if (legacyConfig && !config) {
      config = legacyConfig;
      feedback.warn(
        `Warning: the name '${LEGACY_CONFIG_FILE}' is deprecated; please rename to '${CONFIG_FILE}' soon`
      );
    }
    if (notconfig && !config) {
...
export async function buildStructureParts(
  topLevel: TopLevel
  ...
  const actionsPart = await buildActionsPart(
    packages,
    displayName,
    includer,
    reader
...
export function assembleInitialStructure(
  ...
  configPart.packages =
    actionsPart.packages && configPart.packages
      ? mergePackages(actionsPart.packages, configPart.packages)
      : actionsPart.packages
      ? actionsPart.packages
      : configPart.packages
      ? configPart.packages
      : [];
...
async function readPackage(
  ...
  debug(
    "reading information for package '%s' with display path '%s'",
    pkgPath,
    displayPath
  );
  return reader
    .readdir(pkgPath)
    .then(async (items: PathKind[]) => {
      items = filterFiles(items);
      ...
      for (const item of items) {
        ...
        if (!item.isDirectory) {
          ...
          const { name, runtime, binary, zipped } = await actionFileToParts(
            item.name
          );
          ...
          seen.set(item.name, '*');
          promises.push(
            getBuildForAction(file, reader).then((build) => {
              return {
                name: item.name,
                file,
                displayFile,
...
function duplicateName(actionName: string, formerUse: string, newUse: string) {
  const former =
    formerUse === '*' ? 'as a directory' : `with runtime '${formerUse}'`;
  const present =
    newUse === '*' ? 'as a directory' : `with runtime '${newUse}'`;
  return new Error(
    `The action name '${actionName}' appears twice, once ${former} and once ${present}`
  );
}
  ...
  if (config.packages) {
    ...
    for (const pkg of config.packages) {
      if (includer.isPackageIncluded(pkg.name, false)) {
        if (pkg.actions) {
          pkg.actions = pkg.actions.filter((action) =>
            includer.isActionIncluded(pkg.name, action.name)
          );
        }
        newPkgs.push(pkg);
      }
