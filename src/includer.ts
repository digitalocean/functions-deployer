...
class IncluderImpl implements Includer {
  ...
  isPackageIncluded = (pkg: string, all: boolean) => {
    const nominallyIncluded =
      this.includedPackages.has(pkg) ||
      (this.isExcludingOnly && !this.excludedPackages.has(pkg));
    if (all) {
  ...
  isActionIncluded = (pkg: string, action: string) => {
    ...
    if (
      this.excludedPackages.has(pkg) ||
      this.isActionInMap(pkg, action, this.excludedActions)
    ) {
  ...
  isIncludingEverything = () => {
    return (
      this.isExcludingOnly &&
      this.includedActions.size === 0 &&
      this.includedPackages.size === 0
    );
  };
