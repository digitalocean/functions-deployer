...
export function parseGithubRef(projectPath: string): GithubDef {
  ...
  if (!toParse) {
    throw new Error(
      'internal error: parseGithubRef should not have been called'
    );
  }
  ...
  if (slashSplit.length < 2) {
    throw new Error(
      'too few / characters in GitHub reference; at least <owner>/<repo> is required'
    );
  }
  ...
  const rawAuth = getGithubAuth();
  let [auth, baseUrl] = (rawAuth || '').split('@'); // eslint-disable-line prefer-const
  if (baseUrl) {
...
export async function readContents(
  ...
  def: GithubDef,
  path: string
): Promise<DirectoryItem[] | FileContent> {
  ...
  try {
    contents = await client.repos.getContent(
      ref ? { owner, repo, path, ref } : { owner, repo, path }
    );
  } catch (err) {
    if (err.status === 404) {
      // Common user error
      throw new Error(
        `The repository path '${formatGithubDef(
          def
        )}' is not recognized by GitHub`
      );
    } else if (
      err.status === 403 &&
      ...
      throw new Error(
        `You can't deploy '${formatGithubDef(
          def
        )}' without authenticating to GitHub (requires too high an access rate).`
      );
    } else {
  ...
  if (contents.status !== 200) {
    throw new Error(
      `Reading path '${path}' from ${def.owner}/${def.repo}' failed with status code ${contents.status}`
    );
  }
  if (!contents.data) {
    throw new Error(
      `Reading path '${path}' from ${def.owner}/${def.repo}' succeeded but provided no data`
    );
  }
...
export function seemsToBeProject(data: DirectoryItem[]): boolean {
  if (Array.isArray(data)) {
    ...
    for (const item of items) {
      ...
      if (['packages', 'web'].includes(item.name) && item.type === 'dir')
        return true;
    }
  }
  return false;
  ...
  for (const item of contents) {
    ...
    if (item.type === 'dir') {
      ...
      promise = promise.then(() =>
        fetchDir(client, def, item.path, target, false)
      );
    } else {
