...
export const SYSTEM_EXCLUDE_PATTERNS = [
  '.gitignore',
  '.DS_Store',
  '**/.git/**',
  ...
  return reader.readFileContents(configFile).then(async (data) => {
    ...
    try {
      ...
      if (badVars && badVars.length > 0) {
        ...
        config.error = new Error(
          'The following substitutions could not be resolved: ' + formatted
        );
      }
      return config;
    } catch (err) {
...
export function encryptProjectConfig(config: string): { config: string, key: string } {
...
export function decryptProjectConfig(config: string, key: string): string {
  const iv = Buffer.alloc(16).fill(0);
  const decipher = crypto.createDecipheriv('aes256', Buffer.from(key, 'hex'), iv);
...
async function hasDefaultRemote(
  action: ActionSpec,
  reader: ProjectReader
): Promise<boolean> {
...
export async function getRuntimeForAction(
  action: ActionSpec,
  reader: ProjectReader
): Promise<string> {
  if (action.runtime) {
  ...
  if (pathKind.isFile) {
    const { runtime } = await actionFileToParts(pathKind.name);
    return runtime;
...
export async function agreeOnRuntime(items: string[]): Promise<string> {
  ...
  const partsArray = await Promise.all(
    items.map((item) => actionFileToParts(item))
  );
...
function isValidOwnership(item: any): boolean {
  return (
    isDictionary(item) &&
    (typeof item.production === 'string' || typeof item.test === 'string')
  );
}
...
function transformLegacyTriggers(args: any) {
  ...
  transformedArgs.forEach((trigger, index) => {
    ...
    transformedArgs[index].type =
      trigger.sourceType === 'scheduler' ? 'SCHEDULED' : trigger.sourceType;
...
export function validateTriggers(arg: any): string {
  ...
  for (const trigger of arg as TriggerSpec[]) {
    ...
    if (![TriggerType.SCHEDULED].includes(type as TriggerType)) {
      return `the trigger 'type' field must be '${Object.values(
        TriggerType
      ).join(' | ')}'`;
    }
...
export function combineResponses(responses: DeployResponse[]): DeployResponse {
  ...
  const combinedSuccesses: DeploySuccess[][] = responses.map(
    (response) => response.successes
  );
  const successes = combinedSuccesses.reduce(
    (prev, curr) => prev.concat(curr),
    []
  );
  const combinedFailures: Error[][] = responses.map(
    (response) => response.failures
  );
  const failures = combinedFailures.reduce(
    (prev, curr) => prev.concat(curr),
    []
  );
  const combinedIgnored: string[][] = responses.map(
    (response) => response.ignored
  );
  const ignored = combinedIgnored.reduce((prev, curr) => prev.concat(curr));
  const packageVersions = responses.reduce(
    (prev, curr) => Object.assign(prev, curr.packageVersions),
    {}
  );
  const actionVersions = responses.reduce(
...
export function isTargetNamespaceValid(
  client: Client,
  namespace: string
): Promise<boolean> {
  return getTargetNamespace(client).then((ns) => {
    if (ns === namespace) {
      ...
      throw new Error(
        `Supplied credentials do not match target namespace '${namespace}'; deployment aborted`
      );
    }
  });
}
...
export async function actionFileToParts(fileName: string): Promise<{
  name: string;
  binary: boolean;
  zipped: boolean;
  runtime: string;
...
export async function promiseFilesAndFilterFiles(
  root: string,
  reader: ProjectReader
): Promise<string[]> {
...
async function promiseFiles(
  ...
  while (subdirs.length > 0) {
    ...
    debug(
      "promiseFiles recursing on subdirectory '%s', with '%d' files accumulated and '%d' subdirectories still pending",
      next,
      files.length,
      subdirs.length
    );
...
async function promiseFilesRound(
  dir: string,
  files: string[],
  subdirs: string[],
  reader: ProjectReader
...
export function substituteFromEnvAndFiles(
  input: string,
  envPath: string,
  projectPath: string,
  feedback: Feedback
): { content: string; badVars: string[] } {
  ...
  if (warn) {
    feedback.warn(
      "Using '${}' for dictionary substitution is now deprecated; use '$()'"
    );
  }
  if (badVars.length > 0) {
...
function findNextSymbol(input: string): { index: number; terminator: string } {
  ...
  if (haveBrace) {
    if (haveParen) {
      return nextBrace < nextParen
        ? { index: nextBrace, terminator: '}' }
        : { index: nextParen, terminator: ')' };
    } else {
...
function getDictionarySubstitution(
  tokens: string,
...
function getPropsFromFile(filePath: string): Record<string, string> {
  ...
  const propParser = require('dotenv'); // eslint-disable-line @typescript-eslint/no-var-requires
...
export async function wipe(
  ...
  if (errors.length > 1) {
    const combined = Error(
      'multiple errors occurred while cleaning the namespace'
    ) as any;
...
export function saveUsFromOurselves(
  namespace: string,
  apihost: string
): boolean {
  ...
  return (
    sensitiveNamespaces.includes(namespace) &&
    isProductionProject(apihost, productionProjects)
  );
}
...
function getStatusDir(project: string): {
  statusDir: string;
  created: boolean;
} {
...
export function writeProjectStatus(
  ...
  const versionInfo: VersionEntry = {
    apihost,
    namespace,
    packageVersions,
    actionVersions,
...
export function invokeWebSecure(
  actionAndQuery: string,
  auth: string,
  apihost: string
): Promise<any> {
  const url = apihost + '/api/v1/web' + actionAndQuery;
...
export function renamePackage(
  ...
  if (!pkg) {
    throw new Error(
      `Package '${oldName}' was not found in the DeployStructure`
    );
  }
  pkg.name = newName;
  if (pkg.actions) {
...
export function getExclusionList(): string[] {
  ...
  try {
    ...
    userDefinedPatterns = fs
      .readFileSync(globalExcludeFile)
      .toString()
      .split('\n')
